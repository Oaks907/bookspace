...
/**
 * Create by haifei on 5/12/2018 2:34 PM.
 */
public class BTree2<K, V> {
  ...
  private static class BTreeNode<K, V> {
    ...
    public int compare(K key1, K key2) {
      return kComparator == null ?
        ((Comparable<K>) key1).compareTo(key2) : kComparator.compare(key1, key2);
    }
    ...
    public SearchResult<V> searchKey(K key) {
      ...
      while (low < high) {
        ...
        final int compare = compare(entry.getKey(), key);
        if (compare == 0) {
          break;
        } else if (compare > 0) {
          high = mid - 1;
    ...
    public V putEntry(Entry<K, V> entry) {
      ...
      final SearchResult<V> searchResult = searchKey(entry.getKey());

      if (searchResult.isExist()) {
        final Entry<K, V> findEntry = entrys.get(searchResult.getIndex());
        final V oldValue = findEntry.getValue();
        findEntry.setValue(entry.getValue());
        return oldValue;
      } else {
        // TODO: 5/12/2018 insert操作
        return null;
      }
    }
    ...
    private boolean insertEntry(Entry<K, V> entry) {
      ...
      if (searchResult.isExist()) {
        ...
        insertEntry(entry, searchResult.getIndex());
        return true;
      }
    }
    ...
    public void insertChild(BTreeNode<K, V> child, int index) {
      ...
      List<BTreeNode<K, V>> newList = new ArrayList<>();
      int i = 0;
      for (; i < index; i++) {
  ...
  public BTree2(Comparator<K> comparator) {
    this();
    this.comparator = comparator;
  }

  public BTree2(Comparator<K> comparator, int t) {
    this(t);
    this.comparator = comparator;
  }
  ...
  int compare(K key1, K key2) {
    return comparator == null ? ((Comparable<K>) key1).compareTo(key2) : comparator.compare(key1,
      key2);
  }
  ...
  private V search(BTreeNode<K, V> node, K key) {
    final SearchResult<V> searchResult = node.searchKey(key);
    if (searchResult.isExist()) {
      ...
      if (node.isLeaf()) {
        ...
        search(node.childAt(searchResult.getIndex()), key);
      }
    }
    return null;
  }
  ...
  private void splitNode(BTreeNode<K, V> parentNode, BTreeNode<K, V> childNode, int index) {
    ...
    BTreeNode<K, V> siblingNode = new BTreeNode<>(comparator);
    ...
    final Entry<K, V> entry = childNode.entryAt(t - 1);
  ...
  /**
   * 在一个非满节点中插入给定的项。
   * 每次在当前节点找到要插入的位置。然后找到子节点，继续找插入位置。直到找到叶子节点
   *
   * @param node  - 非满节点
   * @param entry - 给定的项
  ...
  private boolean insertNotFull(BTreeNode<K, V> node, Entry<K, V> entry) {
    ...
    if (node.isLeaf()) {  //如果是叶子节点直接插入。方法内部会设置好位置
       ...
       * 该位置对应的子树中
       */
      final SearchResult<V> searchResult = node.searchKey(entry.getKey());
      //key值已经存在
      if (searchResult.isExist()) {
        return false;
      }
      ...
      BTreeNode<K, V> child = node.childAt(index);
      // 如果子节点是满节点。
      if (child.size() == maxKeySize) {
        //则先分裂
        splitNode(node, child, index);
        /* 如果给定entry的键大于分裂之后新生成项的键，则需要插入该新项的右边，
         * 否则左边。
         */
        if (comparator.compare(entry.getKey(), node.entryAt(index).getKey()) > 0) {
          child = node.childAt(index + 1);
        }
      }
      return insertNotFull(child, entry);
    }
  }
   ...
   * @param key   - 键
   * @param value - 值
   * @return 如果B树中不存在给定的项，否则false
   */
  public boolean insert(K key, V value) {

    // 如果根节点满了，则B树长高
    if (root.size() == maxKeySize) {
      BTreeNode<K, V> newRoot = new BTreeNode<>(comparator);
  ...
  private V putNotFull(BTreeNode<K, V> node, Entry<K, V> entry) {
    ...
    if (node.isLeaf()) {
      ...
      final int index = searchResult.getIndex();

      BTreeNode<K, V> childNode = node.childAt(index);
      //子节点是满节点
      if (childNode.size() >= maxKeySize) {
        //分裂
        splitNode(node, childNode, index);
        if (comparator.compare(entry.getKey(), node.entryAt(index).getKey()) > 0) {
  ...
  private Entry<K, V> delete(BTreeNode<K, V> node, K key) {
    ...
    if (searchResult.isExist()) {
      ...
      if (node.isLeaf()) {
        return node.removeEntry(index);
      } else {
        // 2.a 如果节点node中前于key的子节点包含至少t个项
        BTreeNode<K, V> leftChildNode = node.childAt(index);

        // TODO: 参看：https://luozhong915127.iteye.com/blog/1638116
        //牢记这句话！！！！！
        //>=t 代表去除一个元素后，左子树仍然能够构成一个子树。就提出左子树的最右元素填充删除位置的Node
        if (leftChildNode.size() >= t) {
          // 使用leftChildNode中的最后一个项代替node中需要删除的项
          node.removeEntry(index);
          node.insertEntry(leftChildNode.entryAt(leftChildNode.size() - 1), index);
          // 递归删除左子节点中的最后一个项。因为删除节点的左子树的最后一项被提了上去
          return delete(leftChildNode, leftChildNode.entryAt(leftChildNode.size() - 1).getKey());
        } else {
          //如果左子树不能提出一个元素填充到父节点。就尝试由右子树中提取一个Node
          // 2.b 如果节点node中后于key的子节点包含至少t个关键字
          BTreeNode<K, V> rightChildNode = node.childAt(index + 1);
          if (rightChildNode.size() >= t) {
            // 使用rightChildNode中的第一个项代替node中需要删除的项
            node.removeEntry(index);
            node.insertEntry(rightChildNode.entryAt(0), index);
            // 递归删除右子节点中的第一个项
            return delete(rightChildNode, rightChildNode.entryAt(0).getKey());
          } else {
            //左右节点都不能为父节点提供元素。就合并左右子树，生成 左子树 -- 删除节点 -- 右子树结构，继续进行删除
            // 2.c 前于key和后于key的子节点都只包含t-1个项
            Entry<K, V> deleteEntry = node.removeEntry(index);
            node.removeChild(index + 1);
            // 将node中与key关联的项和rightChildNode中的项合并进leftChildNode
            leftChildNode.addEntry(deleteEntry);
      ...
      final BTreeNode<K, V> childNode = node.childAt(index);

      // 如果子节点有不少于t个项，则递归删除
      if (childNode.size() >= t) {
        return delete(childNode, key);
      } else {
        //子节点小于 t 项,如果删除一个不能保持原来的结构
        BTreeNode<K, V> siblingNode = null;
        int siblingIndex = -1;
        //要删除的索引位置,存在右兄弟节点
        if (index < node.size()) {

          if (node.childAt(index + 1).size() >= t) {
        ...
        if (null != siblingNode) {
          ...
          if (siblingIndex < index) {
            ...
            childNode.insertEntry(node.entryAt(index), childNode.size() - 1);
            node.removeEntry(index);
            node.insertEntry(siblingNode.entryAt(0), index);
          ...
          if (index < node.size()) {  // 存在右兄弟，直接在后面追加
            ...
            for (int i = 0; i < rightSiblingNode.size(); i++) {
              ...
              if (!rightSiblingNode.isLeaf()) {
                for (int j = 0; j <= rightSiblingNode.size(); j++) {
            ...
            //存在左子节点
            BTreeNode<K, V> leftSiblingNode = node.childAt(index - 1);
            childNode.insertEntry(node.entryAt(index - 1), 0);
